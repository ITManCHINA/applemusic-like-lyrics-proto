	...
	return new Proxy(target, {
		get(t, prop, receiver) {
			...
			if (typeof prop === "string" && !Number.isNaN(Number(prop))) {
				return amllLines[Number(prop)];
			}
			if (prop === Symbol.iterator) {
				return () => amllLines[Symbol.iterator]();
			}
			if (typeof (amllLines as any)[prop] === "function") {
				return (amllLines as any)[prop].bind(amllLines);
			}
			if (prop === "length") {
				return amllLines.length;
			}
			return Reflect.get(t, prop, receiver);
		},
	});
...
export function toTTMLResult(
	...
	amllMetadata: (AmllMetadata | { key: string; value: string[] })[],
