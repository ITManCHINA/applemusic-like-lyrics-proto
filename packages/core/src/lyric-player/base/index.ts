	...
	implements HasElement, Disposable
{
	...
	setLyricLines(
		lines:
			| LyricLine[]
			| { lines?: LyricLine[]; lyricLines?: LyricLine[] },
		initialTime = 0,
	): void {
		const targetLines: LyricLine[] = Array.isArray(lines)
			? lines
			: (lines.lines ?? lines.lyricLines ?? []);
		if (import.meta.env.DEV) {
			console.log("设置歌词行", targetLines, initialTime);
		}

		this.dataManager.setOriginalLines(targetLines);
